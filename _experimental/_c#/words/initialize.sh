...
echo "Creating solution 'WordsSolution'..."
...
for i in "${!TEST_PROJECTS[@]}"; do
  ...
  dotnet add test/$test_proj/$test_proj.csproj reference src/$lib_proj/$lib_proj.csproj
  if [ $? -ne 0 ]; then
...
for test_project in "${TEST_PROJECTS[@]}"; do
  ...
  dotnet sln $SOLUTION.sln add test/$test_project/$test_project.csproj
  if [ $? -ne 0 ]; then
