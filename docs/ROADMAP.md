...
| **Indicadores de fallo compatibles con el lenguaje/API** (sin excepciones): la función devuelve una representación controlada (`-1`, `None`, `null`, `Option/Maybe`, `Result`, etc.) sin interrumpir el flujo / **Language/API-compatible failure indicators** (no exceptions): function returns a controlled representation (`-1`, `None`, `null`, `Option/Maybe`, `Result`, etc.) without interrupting flow | 1 — **Algorithms Pure** | `naive_sort`, `data_structures_basics`, `data_structures_advanced`, `searching`, `efficient_sort`, `distributed_sort` |
| **Contrato explícito sin contrato aparte**: la API del módulo se declara en la notación natural del lenguaje y la representación queda oculta; no se exige `interface`/`trait`/`protocol` mientras haya una sola implementación / **Explicit contract without a separate contract type**: the module API is declared in the language's natural notation and the representation stays hidden; `interface`/`trait`/`protocol` is not required while there is a single implementation | 1 — **Algorithms Pure** | `data_structures_basics`, `data_structures_advanced`, `searching` |
...
| **Contratos abstractos**: `interface`, `trait`, `protocol`, clase abstracta o firma de módulo cuando un algoritmo consume estructuras intercambiables (cola de prioridad, pila, cola) / **Abstract contracts**: `interface`, `trait`, `protocol`, abstract class or module signature when an algorithm consumes interchangeable structures (priority queue, stack, queue) | 3 — **Algorithms on Structures** | `graph_algorithms`, `backtracking`, `greedy` |
...
| **Abstracción formal**: varias implementaciones reales de la misma abstracción y contratos de puerto para persistencia y parsing / **Formal abstraction**: several real implementations of the same abstraction and port contracts for persistence and parsing | 4 — **Abstraction & Persistence** | `modeling`, `parsing`, `data_base` |
...
>
> **ES:** El contrato es normativo y global; la notación para declararlo es idiomática. El calendario de adopción de contratos abstractos (`interface`, `trait`, `protocol`, firma de módulo) y las familias de notación por lenguaje se detallan en [`AGENT_Template.md`](AGENT_Template.md).
> **EN:** The contract is normative and global; the notation used to declare it is idiomatic. The adoption calendar for abstract contracts (`interface`, `trait`, `protocol`, module signature) and the notation families per language are detailed in [`AGENT_Template.md`](AGENT_Template.md).
...
| 10 — searching | $O(n)$ a $O(\log n)$, más preparación cuando aplica | array numérico; acceso indexado | consumo de una secuencia ordenada; contrato y composición con sort |
...
| Módulo | Estado | Notas |
|--------|--------|-------|
| `core.text.transformations` | 🔄 | representación de strings, recorrido, reverse, trim, case, padding y normalización de blancos | [`📄 11_Transformations.md`](core/text/11_Transformations.md) |
| `core.text.patterns` | 📋 | palindrome, anagram, búsqueda ingenua y patrones literales | [`📄 12_Patterns.md`](core/text/12_Patterns.md) |
| `core.text.substr` | 📋 | LPP, KMP, Boyer–Moore y Z; tablas auxiliares basadas en arrays | [`📄 13_Substr.md`](core/text/13_Substr.md) |
| `core.text.input_output` | 📋 | fixtures compartidos, read/write/append, codificación y excepciones de frontera | [`📄 14_Input_Output.md`](core/text/14_Input_Output.md) |
| `core.text.etl_basico` | 📋 | CSV delimitado, validación, transformación, búsqueda/ordenamiento y salida reproducible | [`📄 15_ETL_Basico.md`](core/text/15_ETL_Basico.md) |
...
| `core.structures.dynamic_programming` | 📋 | LCS, knapsack 0/1, coin change, LIS, caminos en grid | [`📄 18_Dynamic_Programming.md`](core/structures/18_Dynamic_Programming.md) |
