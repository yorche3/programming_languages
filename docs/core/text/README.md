...
|--------|--------|------|----------------|
| `transformations` | 🔄 | representación, reverse, trim, case, padding y blancos | [`11_Transformations.md`](11_Transformations.md) |
| `patterns` | 📋 | palindrome, anagram, búsqueda ingenua y comodines opcionales | [`12_Patterns.md`](12_Patterns.md) |
| `substr` | 📋 | LPS, KMP, Z y Boyer–Moore opcional; LCS va a DP | [`13_Substr.md`](13_Substr.md) |
| `input_output` | 📋 | fixtures comunes, read/write/append y excepciones de frontera | [`14_Input_Output.md`](14_Input_Output.md) |
| `etl_basico` | 📋 | CSV delimitado, validación, transformaciones y reuse de searching/sort | [`15_ETL_Basico.md`](15_ETL_Basico.md) |

> **ES:** `transformations` aún **no se ha refactorizado** a la estructura homologada `core/text/` (0/50 submódulos registrados en `.gitmodules`). Esta es la primera fase que introduce strings: define la unidad indexable de texto y los recorridos antes de `naive_find` y los algoritmos de subcadena. La búsqueda sobre strings no se presupone en `searching`, que trabaja exclusivamente con secuencias indexables de números. LCS se enseña en programación dinámica y regex completo se reserva para Fase 4. Los fixtures de entrada y salida se versionan para evitar diferencias entre runners y sistemas operativos.
> **EN:** `transformations` has **not yet been refactored** to the standardized `core/text/` layout (0/50 submodules registered in `.gitmodules`). This is the first phase that introduces strings: it defines the indexable text unit and traversals before `naive_find` and substring algorithms. String search is not assumed in `searching`, which works exclusively with indexable sequences of numbers. LCS is taught in dynamic programming and full regex is reserved for Phase 4. Input and output fixtures are versioned to avoid runner and operating-system differences.
