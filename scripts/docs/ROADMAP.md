...
| L6.5 | Perfiles de modelo por encargo (BYOK): `model:` en cada plantilla y catálogo de perfiles | 0.11.0 |
...
| 0.11.0 | L6.5 | 4–6 | **Perfiles de modelo por encargo**: `model:` en el frontmatter de cada plantilla y un catálogo de perfiles (BYOK, con DeepSeek para `implement` y un modelo económico para validar y documentar) | La delegación y la validación ya existen; el modelo es la palanca de calidad y de coste, y va después de tenerlas |
...
| Perfiles de modelo | Versión propia: **L6.5 → 0.11.0**, con L7 desplazada a la 0.12.0. La capa formaliza lo que BYOK ya permite a mano (`COPILOT_PROVIDER_*`), con `model:` en el frontmatter de cada plantilla y sin claves en el repositorio |
