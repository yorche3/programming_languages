...
# Desde la v1.1.0 `new` ejecuta la **secuencia de inicialización** del lenguaje (pasos,
    ...
    if root="$(_glot_repo_root)"; then
        ...
        if [[ -n "$install_dir" && -r "$install_dir/glot.sh" ]]; then
            ...
            if [[ -r "$install_meta" ]]; then
                ...
                install_data_sha="$(sed -n 's/^data_sha=//p' "$install_meta" | head -1)"
            fi
            ...
            if [[ -n "$install_data_sha" && "$install_data_sha" != "-" ]]; then
                if [[ "$(_glot_install_fingerprint "$install_dir" data || true)" == "$install_data_sha" ]]; then
                    printf 'install_data: ok (%s)\n' "$install_data_sha"
                else
                    printf 'install_data: stale (%s)\n' "$install_data_sha"
                fi
            else
                printf 'install_data: (sin metadatos / no metadata)\n'
            fi
        else
            printf 'install: no (%s)\n' "$install_dir"
        fi
...
# _glot_install_files <dir> [script|data] — lo que viaja con la copia, en orden estable:
# el script, los datos, las plantillas y los completados. `script` deja solo el script;
# `data`, todo lo demás; sin grupo, lo pide todo.
_glot_install_files() {
    local dir="$1"
    local group="${2:-all}"
    local item=""
    ...
    if [[ "$group" != "script" ]]; then
        for item in "$dir"/data/* "$dir"/prompts/* "$dir"/completions/*; do
            [[ -f "$item" ]] || continue
            printf '%s\n' "$item"
        done
    fi
...
# _glot_install_fingerprint <dir> [script|data] — huella de lo instalado. Desde la v1.1.0
# `install` guarda la huella **completa** y no solo la de `glot.sh`: una copia con el
# `data/` o las plantillas viejos deja de pasar por buena (medido el 2026-09-25: la copia
# seguía diciendo `install_stale: no` con el comando de pruebas de Ada anterior).
_glot_install_fingerprint() {
    local dir="$1"
    local group="${2:-all}"
    local files=""
    local item=""

    files="$(_glot_install_files "$dir" "$group" | LC_ALL=C sort)"
    [[ -n "$files" ]] || return 1

    while IFS= read -r item; do
        printf '%s\t%s\n' "${item#"$dir"/}" "$(_glot_install_sha "$item" || printf '%s' '-')"
    done <<<"$files" | cksum | awk '{printf "%08d", $1}'
}
...
_glot_cmd_install() {
    ...
    # metadatos: con esto `doctor` puede decir si la copia se quedó vieja. La huella `sha`
    # cubre todo lo que viaja con la copia, y `data_sha` deja aislada la parte de datos,
    # plantillas y completados, que es la que se olvida al reinstalar.
    {
        printf 'version=%s\n' "$GLOT_VERSION"
        printf 'source=%s\n' "$src"
        printf 'date=%s\n' "$(date -Iseconds)"
        printf 'sha=%s\n' "$(_glot_install_fingerprint "$dir" || printf '%s' '-')"
        printf 'script_sha=%s\n' "$(_glot_install_fingerprint "$dir" script || printf '%s' '-')"
        printf 'data_sha=%s\n' "$(_glot_install_fingerprint "$dir" data || printf '%s' '-')"
